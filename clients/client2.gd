...
func _ready():
	random_wait = randi() % 10 + 1

func _process(delta):
	if dir.client_2_order == 0:
		if dir.client_2_timer > random_wait:
			dir.client_2_order = randi() % 4 + 1
			dir.client_2_timer = 0
	dir.client_2_timer += delta
	if dir.client_2_order != 0:
		match dir.client_2_order:
			1:
...
func beer_passed():
	if get_tree().get_nodes_in_group(str("beer_order_2", self)).size() > 0:
		var current_beer = get_tree().get_nodes_in_group(str("beer_order_2", self)).pop_front()
		remove_child(current_beer)
	
func display_beer_order(beer_order):
	var current_beer = beer_order.instance()
	current_beer.add_to_group(str("beer_order_2", self))
	add_child(current_beer)
	if get_tree().get_nodes_in_group(str("beer_order_2", self)).size() > 1:
