...
func _ready():
	random_wait = randi() % 50 + 1

func _process(delta):
	if dir.client_3_order == 0:
		if dir.client_3_timer > random_wait:
			dir.client_3_order = randi() % 4 + 1
			dir.client_3_timer = 0
	dir.client_3_timer += delta
	if dir.client_3_order != 0:
		match dir.client_3_order:
			1:
...
func display_beer_order(beer_order):
	...
	current_beer.add_to_group(str("beer_order_3", self))
	add_child(current_beer)
	if get_tree().get_nodes_in_group(str("beer_order_3", self)).size() > 1:
